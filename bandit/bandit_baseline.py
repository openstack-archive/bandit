...
# To do this is checks out the parent commit, runs Bandit (with any provided
...
def main():
    ...
    try:
        ...
        logger.info('Got current commit: [%s]', commits[0].name_rev)

        parent_commit = commits[1].hexsha
        logger.info('Got parent commit: [%s]', commits[1].name_rev)
    ...
    steps = [{'message': 'Getting Bandit baseline results',
              ...
              'args': bandit_args + ['-f', 'json', '-o', baseline_tmp_file]},

             {'message': 'Comparing Bandit results to baseline',
              'commit': current_commit,
              'args': bandit_args + ['-b', baseline_tmp_file] + output_type}]
    ...
    for step in steps:
        ...
        return_code, output, err = call_command(bandit_command)

        if return_code not in [0, 1]:
            logger.error("Error running command: %s\nOutput: %s\nError: %s",
                         bandit_args, output, err)
...
###################### Utility function to run Bandit commands ################
...
def initialize():
    ...
    if repo.is_dirty():
        logger.error("Current working directory is dirty and must be resolved")
        valid = False
