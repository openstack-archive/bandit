...
"""
Bandit Baseline is a tool that runs Bandit against a Git branch, and compares
the current commit findings to the parent commit findings.

To do this is checks out the parent branch, runs Bandit (with any provided
filters or profiles), checks out the current branch, runs Bandit, and then
...
def main():
    ...
    # return status of last Bandit run
    sys.exit(return_code)
...
def determine_output_format():
    ...
    # otherwise make sure valid output format is given
        ...
        f_arg_loc = bandit_args.index('-f')

        # check for -f with no argument
        try:
            format_arg = bandit_args[f_arg_loc + 1]
        except IndexError:
