...
class BanditManager():
    ...
    def discover_files(self, targets, recursive=False):
        ...
        excluded_paths = self.b_conf.get_option('exclude_dir') or []
        extensions = self.b_conf.get_option('include') or '.py'

        # build list of files we will analyze
        for _, fname in enumerate(targets):
            ...
            if os.path.isdir(fname):
                ...
                if _is_file_included(fname, extensions, excluded_paths,
                                     do_enforce_extensions=False):
...
def _get_files_from_dir(files_dir, extensions='.py', excluded_paths=None):
    ...
    for root, subdirs, files in os.walk(files_dir):
        for filename in files:
            ...
            if _is_file_included(path, extensions, excluded_paths):
                files_list.add(path)
            else:
...
def _is_file_included(path, parsed_extensions, excluded_paths,
                      do_enforce_extensions=True):
    ...
    # in an excluded path
    if file_ext in parsed_extensions or not do_enforce_extensions:
