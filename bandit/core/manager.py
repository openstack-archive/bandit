    ...
    def discover_files(self, targets, recursive=False):
        ...
        for _, fname in enumerate(targets):
            ...
            if os.path.isdir(fname):
                ...
                if _is_file_included(fname, included_globs, excluded_paths,
...
def _get_files_from_dir(files_dir, included_globs='*.py', excluded_paths=None):
    ...
    for root, subdirs, files in os.walk(files_dir):
        for filename in files:
            ...
            if _is_file_included(path, included_globs, excluded_paths):
                files_list.add(path)
            else:
...
def _is_file_included(path, included_globs, excluded_paths,
    ...
    # excluded path
    if _matches_glob_list(path, included_globs) or not do_enforce_extensions:
