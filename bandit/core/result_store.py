...
class BanditResultStore():
    ...
    def _report_xml(self, file_list, scores, excluded_files):
        ...
        XML output can be used to integrate bandit in Jenkins.
        ...
        for filename, issues in items:
            for issue in issues:
                testcase = ET.SubElement(root, 'testcase', classname=filename, name=issue['test'])
                if self._check_severity(issue['issue_severity']):
                    ...
                    ET.SubElement(testcase, 'error', type=issue['issue_severity'],
