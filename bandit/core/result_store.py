...
class BanditResultStore():
    ...
    def _write_report(self, files_list, scores, excluded_files):
        formatter_name = self.format
        formatters_mgr = extension_loader.MANAGER.formatters_mgr
        try:
            formatter = formatters_mgr[formatter_name]
        except KeyError:  # Unrecognized format, so use text
            formatter_name = 'txt'
            formatter = formatters_mgr[formatter_name]

        if formatter == 'csv':
            self.max_lines = 1
        elif formatter_name == 'txt' and self.out_file:
            self.format = 'plain'
