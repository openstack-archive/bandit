...
class BanditResultStore():
    ...
    def _report_text(self, files_list, scores, excluded_files):
        ...
        counts = {
            'severity': { key: 0 for key in constants.RANKING },
            'confidence': { key: 0 for key in constants.RANKING }
        }
        test_counts = {}
        for filename, issues in self.resstore.items():
            for issue in issues:
                counts['severity'][issue['issue_severity']] += 1
                counts['confidence'][issue['issue_confidence']] += 1
                if issue['test'] not in test_counts:
                    test_counts[issue['test']] = {
                        'severity': { key: 0 for key in constants.RANKING },
                        'confidence': { key: 0 for key in constants.RANKING }
                    }
                test_counts[issue['test']]['severity'][issue['issue_severity']] += 1
                test_counts[issue['test']]['confidence'][issue['issue_confidence']] += 1
        self.logger.debug('Total counts: %s' % counts)
        for test in test_counts:
            print("\n\t%s :: %s" % (test, test_counts[test]))
