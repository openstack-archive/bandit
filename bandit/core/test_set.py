...
class BanditTestSet():
    ...
    def _get_filter(self, profile):
        ...
        def _get_id(name):
            ...
            if name in extman.plugins_by_id or name in builtins:
                return name
            return extman.get_plugin_id(name)

        inc = [_get_id(i) for i in (profile.get('include') or [])]
        exc = [_get_id(e) for e in (profile.get('exclude') or [])]
        filtered = inc or extman.plugins_by_id.keys()
        ...
        return [f for f in filtered if f not in exc]
