...
def main():
    ...
    if not config_file:
        ...
        config_locations = [s + DEFAULT_TEST_CONFIG for s in config_dirs]

        for c_file in config_locations:
            if os.path.isfile(c_file):
                if c_file.startswith("./"):
                    warnings.warn("Using relative path for config: %s"
                                  % c_file)
                config_file = c_file  # Found a valid config
                break

    if not config_file:
        logger.error("no config found, tried: %s", ', '.join(config_locations))
        sys.exit(2)
