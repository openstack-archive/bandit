...
/builddir/build/BUILDROOT/OpenLP-2.4.6-2.fc28.x86_64/usr/lib/python3.6/site-packages/openlp/core/utils/__init__.py:405: \
 B310[bandit]: MEDIUM: Audit url open for permitted schemes. \
...
def report(manager, fileobj, sev_level, conf_level, lines=-1, template=None):
    ...
    :param template: Output template with non-terminal symbols N: <N>
                    (default: <abspath>:<line>: <test_id>[bandit]: <severity>: <msg>)
    """
    ...
    for defect in results:
        ...
        for tag in re.findall(pattern, msg_template):
            if tag in non_terminals.keys():
                ...
                LOG.warning("  Tag `{}` was not recognized and will not be expanded".format(tag))
