...
def evaluate_var(sql_var, parent, until, ignore_nodes=None):
    secure = False
    if isinstance(sql_var, ast.Name):
        if isinstance(parent, ast.FunctionDef):
            for name in parent.args.args:
                if name.id == sql_var.id:
                    return False  # Params are not secure

        analyser = DeepAssignation(sql_var, ignore_nodes)
...
def check_risk(node):
    ...
    if isinstance(sql_var, ast.Name):
        ...
        if not is_param:
            secure = evaluate_var(sql_var, parent, node.lineno)
    elif isinstance(sql_var, ast.Call):
        parent = node.parent
        while not isinstance(parent, (ast.Module, ast.FunctionDef)):
            parent = parent.parent
        secure = evaluate_call(sql_var, parent)
    elif isinstance(sql_var, ast.BinOp):
        is_mod = isinstance(sql_var.op, ast.Mod)
        is_left_str = isinstance(sql_var.left, ast.Str)
        if is_mod and is_left_str:
            parent = node.parent
            while not isinstance(parent, (ast.Module, ast.FunctionDef)):
                parent = parent.parent
            new_call = transform2call(sql_var)
