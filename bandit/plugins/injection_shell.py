...
@takes_config('blacklist_functions.process_name_sets.subprocess')
@checks('Call')
def subprocess_popen_with_shell_equals_true(context, config):
    if config and context.call_function_name_qual in config:
...
def any_other_function_with_shell_equals_true(context, config):
    ...
    '''
    if config and context.call_function_name_qual not in config:
        if context.check_call_arg_value('shell', 'True'):
...
@takes_config('blacklist_functions.process_name_sets.shell')
@checks('Call')
...
@takes_config('blacklist_functions.process_name_sets.no_shell')
@checks('Call')
