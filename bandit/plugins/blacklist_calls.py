...
def _get_tuple_for_item(blacklist_object):
    ...
    if 'qualnames' in blacklist_object:
        qualnames = _parse_config_as_list(blacklist_object['qualnames'])
    if 'names' in blacklist_object:
        names = _parse_config_as_list(blacklist_object['names'])
    ...
    if 'params' in blacklist_object:
        params = _parse_config_ast_list(blacklist_object['params'])

    return_tuple = (qualnames, names, message, level, params)
    return return_tuple


def _parse_config_as_list(config):
    """Parse a comma-delimited YAML string as a list, if necessary."""
    if isinstance(config, basestring):
        return [name.strip() for name in config.split(',')]
    else:
        return config
