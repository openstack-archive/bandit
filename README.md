...
Bandit against. If you're project is only compatible with Python 2.7, you
...
analyze your Python code. The `ast` module can only parse that is valid for
the version of Python it is running from. In other words, if you try to use
Python 2.7's `ast` module to parse code written for 3.4 that uses `yield from`
with asyncio, then you'll have syntax errors that will prevent Bandit from
working properly. Alternatively, if you're relying on 2.7's octal notation of
`0777` then you'll have a syntax error if you run Bandit on 3.4.
