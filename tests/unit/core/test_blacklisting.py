...
class BlacklistingTests(testtools.TestCase):
    def test_report_issue(self):
        ...
        self.assertEqual(issue_dict['test_id'], 'B000')
        self.assertEqual(issue_dict['issue_severity'], 'HIGH')
        self.assertEqual(issue_dict['issue_confidence'], 'HIGH')
        self.assertEqual(issue_dict['issue_text'], 'test name')
    ...
    def test_report_issue_defaults(self):
        ...
        issue_dict = issue.as_dict(with_code=False)
        self.assertIsInstance(issue_dict, dict)
        self.assertEqual(issue_dict['test_id'], 'LEGACY')
        self.assertEqual(issue_dict['issue_severity'], 'MEDIUM')
        self.assertEqual(issue_dict['issue_confidence'], 'HIGH')
        self.assertEqual(issue_dict['issue_text'], 'test name')
